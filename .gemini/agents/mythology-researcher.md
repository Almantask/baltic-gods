...
Specialized agent/skill focused on finding, verifying, and formatting Baltic mythology tales and deities with high academic precision.
...
Add the new metadata to `src/content/stories/meta.ts` (or `deities/meta.ts`). Include:
...
- Use the `StoryMeta` interface found in `src/types/content.ts`.
...
## Sources
Refer to `ai/skills/mythology_researcher/resources/sources.md` for primary archives.